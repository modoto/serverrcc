...
const loadCameras = require('./cameras');
...
(async () => {
  ...
  console.log('myCameras----->', myCameras);
  ...
  process.on("message", (msg) => {
    if (msg.type === "command") {
      ...
      if (action === "start_all") {
        ...
        Object.values(cameraManagers).forEach(cam => cam.start());
      }

      // 🔥 STOP ALL
      if (action === "stop_all") {
        console.log("Stopping ALL cameras...");
        Object.values(cameraManagers).forEach(cam => cam.stop());
      }
    }
  });
})();
